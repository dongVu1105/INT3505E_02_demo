 ...
 * - Authentication: Session-based (stored on server, cookie on client)
 * 
 * REST Principles Applied:
 * - Stateful: Session stored on server, cookie sent with every request
...
document.getElementById('loginForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok && result.success) {
            // Lưu username vào localStorage (chỉ để hiển thị)
            localStorage.setItem('username', result.data.username);
            
            // Hiển thị thông báo
            document.getElementById('tokenValue').textContent = 'Session stored on server';
            document.getElementById('tokenDisplay').style.display = 'block';
            
            showAlert('Login successful! Session stored on server.', 'success');
 ...
 * - Cookie tự động được gửi kèm với mỗi request (credentials: 'include')
 * - Server verify session từ cookie
...
async function apiRequest(endpoint, options = {}) {
    ...
    const config = {
        ...options,
        credentials: 'include', // Quan trọng: Gửi cookie với mọi request
        headers: {
            'Content-Type': 'application/json',
            ...options.headers
        }
