"""
REST API Routes
Triển khai REST API với Session-based Authentication
"""
...
def login():
    ...
    - Lưu username vào Flask session (server-side)
    - Session ID được gửi về client qua cookie
    - Cookie tự động gửi kèm với mọi request
    """
    ...
    session.permanent = True  # Session có thời hạn theo config
...
def verify_session_endpoint(current_user):
    """
    GET /api/auth/verify
    Kiểm tra session có hợp lệ không
    
    Session Authentication:
    - Verify từ session stored on server
    - Cacheable: Cache ngắn (60s) với private cache
    """
    return jsonify({
        'success': True,
        'data': {
...
def logout():
    ...
    Session Authentication:
    - Xóa session data trên server
    - Cookie vẫn tồn tại nhưng session ID không còn valid
    """
    session.clear()
